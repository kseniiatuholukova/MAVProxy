...
def timestring(msg):
    ...
    return (
        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(msg._timestamp))
        + ".%.03u" % ts_ms
    )
    ...
    def __init__(self):
        ...
        self.settings = MPSettings(
            [
                ...
                MPSetting(
                    "vehicle_type", str, "Auto", "force vehicle type for mode handling"
                ),
            ]
        )
...
def setup_menus():
    ...
    TopMenu.add(
        MPMenuSubMenu(
            "Tools",
            items=[
                MPMenuItem("MagFit", "MagFit", "# magfit"),
    ...
    for g in root.graph:
        ...
        if load_all:
            if not name in names:
                ...
                ret.append(
                    GraphDefinition(
                        name, expressions[0], description, expressions, filename
                    )
                )
            names.add(name)
            continue
...
def load_graphs():
    ...
    try:
        ...
        for f in dlist:
            raw = pkg_resources.resource_stream(
                "MAVProxy", "tools/graphs/%s" % f
            ).read()
...
def check_vehicle_type():
    ...
    vtypes = {
        "Rover": mavutil.mavlink.MAV_TYPE_GROUND_ROVER,
...
def cmd_graph(args):
    ...
    if args[0][0] == ":":
        ...
        mestate.last_graph = GraphDefinition(
            mestate.settings.title, expression, "", [expression], None
        )
...
def cmd_map(args):
    ...
    mfv_mav_ret = mavflightview.mavflightview_mav(
        mestate.mlog, options, mestate.flightmode_selections
    )
...
def cmd_fft(args):
    ...
    fft_tool = mav_fft.MavFFT(mlog=mestate.mlog, xlimits=xlimits)
    fft_tool.start()
...
def cmd_magfit(args):
    ...
    mfit_tool = magfit.MagFit(title="MagFit", mlog=mestate.mlog, xlimits=xlimits)
    mfit_tool.start()
...
def save_process(
    MAVExpLastGraph, child_pipe_console_input, child_pipe_graph_input, statusMsgs
):
...
def cmd_save(args):
    ...
    child = multiproc.Process(
        ...
        args=[
            mestate.last_graph,
            mestate.child_pipe_send_console,
            mestate.child_pipe_send_graph,
            mestate.status.msgs,
        ],
    )
    child.start()
...
def cmd_messages(args):
    ...
    while True:
        ...
        if (
            hasattr(m, "id") and hasattr(m, "chunk_seq") and m.chunk_seq != 0
        ):  # assume STATUSTEXT
...
def cmd_param_save(args, changed_only=False):
    ...
    for p in k:
        ...
        if (
            changed_only
            and p in mlog.param_defaults
            and mlog.params[p] == mlog.param_defaults[p]
        ):
            continue
        if fnmatch.fnmatch(p, wildcard.upper()):
            v = mlog.params[p]
...
def cmd_devid(args):
    ...
    for p in k:
        if (
            p.startswith("COMPASS_DEV_ID")
            or p.startswith("COMPASS_PRIO")
            or (p.startswith("COMPASS") and p.endswith("DEV_ID"))
        ):
...
def loadfile(args):
    ...
    mlog = mavutil.mavlink_connection(
        args, notimestamps=False, zero_time_base=False, progress_callback=progress_bar
    )
...
def print_caught_exception(e):
    if sys.version_info[0] >= 3:
        ret = "%s\n" % e
        ret += "".join(traceback.format_exception(type(e), value=e, tb=e.__traceback__))
        print(ret)
    else:
    ...
    while True:
        ...
        for i in range(0, len(grui)):
            ...
            if xlim is not None and mestate.settings.sync_xzoom:
                ...
                tzofs = localtimezone.utcoffset(
                    datetime.datetime.utcfromtimestamp(tbase)
                ).total_seconds()
